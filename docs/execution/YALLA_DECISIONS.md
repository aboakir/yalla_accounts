...
## P07 decision — intake is not approval or accounting
1. `AddRepairScreen` remains the stable entry-point class so existing direct `MaterialPageRoute` and named-route callers converge on one P07 implementation.
2. P07 ends after documented intake and save. Estimate lines, quote approval, work-order execution and quality flow belong to P09; accounting truth belongs to P10+.
3. A newly received repair is saved as workflow `QUOTE` and vehicle state `بانتظار الإصلاح`, with zero initial file value and ledger disabled. P07 must not manufacture an invoice, approval, payment or GL posting.
4. Intake photos and customer signature are persisted under the app-support sandbox before their paths are stored in SQLite.
5. Intake DB fields are additive only. P07 keeps DB v69 and uses `RepairTables.ensureP07IntakeSchema()` with guarded `ALTER TABLE ADD COLUMN`, matching the established current-v69 compatibility-bootstrap pattern; existing repair rows and financial data are never rebuilt or deleted.
6. Repair creation and its sync Outbox envelope remain in one SQLite transaction under the P04 local-first contract.
7. On 2026-09-02 Luay explicitly ordered `P07 نفذ` after being informed that C02 human iPhone test was pending. This authorizes starting P07 implementation but does not convert C02 to PASS and does not waive P07 local/visual acceptance gates.
