...
void main() {
  ...
  test(
    'SEC.006 migrates live DB v64 to v65 without historical rewrites',
    () async {
      ...
      try {
        ...
        final activationRows = sq.Sqflite.firstIntValue(
              await db
                  .rawQuery('SELECT COUNT(*) FROM license_activation_state'),
            ) ??
            -1;
        expect(activationRows, 0,
            reason:
                'SEC.006 migration must not fabricate an online activation.');
    ...
    skip: (Platform.environment['YALLA_SEC006_LIVE_DB_PATH']?.trim().isEmpty ??
            true)
        ? 'Historical fixture test; set YALLA_SEC006_LIVE_DB_PATH explicitly to run'
        : false,
  );
}
