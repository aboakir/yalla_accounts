...
void main() {
  ...
  test(
    'SEC.010 leaves live client DB v67 and accounting/auth data unchanged',
    () async {
      final path = Platform.environment['YALLA_SEC010_LIVE_DB_PATH']!.trim();
      ...
      try {
        ...
        final permissions = firstInt(
            await db.rawQuery('SELECT COUNT(*) FROM auth_permissions'));
        final gl =
    ...
    skip: (Platform.environment['YALLA_SEC010_LIVE_DB_PATH']?.trim().isEmpty ??
            true)
        ? 'Historical v67 fixture; default suite must not touch customer DB'
        : false,
  );
}
