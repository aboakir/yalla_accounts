...
void main() {
  group('P18 release readiness FIX1', () {
    test('Android release manifest uses scoped storage and production internet',
        () {
    ...
    test('Android targets API 36 and release never falls back to debug signing',
        () {
    ...
    test('store distribution externalizes legal URLs and gates onboarding CTA',
        () {
      final config = _read('lib/core/release/release_distribution_config.dart');
      final login = _read('lib/features/auth/screens/login_screen.dart');
      final subscription =
          _read('lib/features/subscription/screens/subscription_screen.dart');

      expect(config, contains('YALLA_STORE_DISTRIBUTION'));
      expect(config, contains('YALLA_PRIVACY_URL'));
      expect(config, contains('YALLA_TERMS_URL'));
