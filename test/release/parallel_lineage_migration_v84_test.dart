...
void main() {
  ...
  for (final lineage in ['commercial83', 'owner78']) {
    test('v84 upgrades authentic $lineage without changing financial facts',
        () async {
      final directory =
          await Directory.systemTemp.createTemp('yallah_lineage_');
      final dbPath = '${directory.path}/fixture.db';
      final fixture = 'test/fixtures/migration/$lineage';
      await File(dbPath).writeAsBytes(
          gzip.decode(await File('$fixture.db.gz').readAsBytes()));
      ...
      try {
        ...
        for (final table in ['clients', 'gl_entries', 'gl_lines']) {
          final expected = (before[table] as List).cast<Map<String, dynamic>>();
          ...
          for (var i = 0; i < expected.length; i++) {
            for (final field in expected[i].entries) {
              expect(actual[i][field.key], field.value,
        ...
        for (var i = 0; i < cheques.length; i++) {
          for (final field in [
            ...
            expect(cheques[i][field], oldCheques[i][field],
        ...
        final context =
            await db.rawQuery('PRAGMA table_info(sync_mutation_context)');
        ...
        final financialBeforeRestart =
            await db.query('gl_lines', orderBy: 'id');
        await db.close();
        DatabaseMigration.useDatabaseForTesting(null);
        db = await DatabaseMigration.initDatabase(pathOverride: dbPath);
        expect(
            await db.query('gl_lines', orderBy: 'id'), financialBeforeRestart);
  ...
  test('v84 refuses a future schema without downgrading or resetting data',
      () async {
    final directory =
        await Directory.systemTemp.createTemp('yallah_future_schema_');
    final dbPath = '${directory.path}/fixture.db';
    await File(dbPath).writeAsBytes(gzip.decode(
        await File('test/fixtures/migration/commercial83.db.gz')
            .readAsBytes()));
    try {
      ...
      await db.close();
      await expectLater(DatabaseMigration.initDatabase(pathOverride: dbPath),
