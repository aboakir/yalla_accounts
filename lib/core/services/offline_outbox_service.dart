...
class OfflineOutboxService {
  ...
  static Future<int> pendingCount(DatabaseExecutor db) async {
    return (await queueStats(db)).pending;
  }

  static Future<OutboxQueueStats> queueStats(DatabaseExecutor db) async {
    final result = await db.rawQuery(
      "SELECT "
      "SUM(CASE WHEN sent = 0 AND status IN ('pending','failed') "
      "THEN 1 ELSE 0 END) AS pending, "
      "SUM(CASE WHEN sent = 0 AND status = 'failed' "
      "THEN 1 ELSE 0 END) AS failed, "
      "SUM(CASE WHEN sent = 0 AND status = 'sending' "
      "THEN 1 ELSE 0 END) AS sending "
      "FROM ${TechnicalTables.outboxTable}",
    );

    final row = result.isEmpty ? const <String, Object?>{} : result.first;
