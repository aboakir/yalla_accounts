...
// - فلاتر: نطاق تاريخ (يُطبّق داخل LEFT JOIN على gl_entries) + بحث باسم/كود الحساب.
// - تجميع: REVENUE / EXPENSE + صافي الربح/الخسارة.
// - عرض: جدولي على الديسكتوب + بطاقات على الموبايل.
// - تصدير: CSV + PDF (A4، RTL).
// - Drill-through: فتح GL للحساب المحدد وتمرير نفس الفلاتر.
// - خيار "إظهار الصفرية" لإظهار الحسابات بلا حركة.
// - بدون إنشاء جداول وهمية.
//
// ملاحظات PDF:
// أضف الخط إلى pubspec لو أردت دعم عربي:
// flutter:
//   assets:
...
class _IncomeStatementScreenState extends State<IncomeStatementScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final q = _query.trim();
      // دعم النوع أو الكود: 4xxx للإيرادات و 5xxx للمصاريف حتى لو type مختلف بالـ seeding
