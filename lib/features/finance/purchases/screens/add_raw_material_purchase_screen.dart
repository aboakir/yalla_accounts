...
class AddRawMaterialPurchaseScreen extends StatefulWidget {
  const AddRawMaterialPurchaseScreen({super.key});

  static Future<void> open(BuildContext context) async {
    await Navigator.of(context).push(
      MaterialPageRoute(builder: (_) => const AddRawMaterialPurchaseScreen()),
    );
  }

  @override
  State<AddRawMaterialPurchaseScreen> createState() =>
      _AddRawMaterialPurchaseScreenState();
}

class _AddRawMaterialPurchaseScreenState
    extends State<AddRawMaterialPurchaseScreen> {
  final _formKey = GlobalKey<FormState>();

  final _supplierIdCtrl = TextEditingController();
  final _noteCtrl = TextEditingController(text: 'مواد خام');

  DateTime _date = DateTime.now();
  String _method = 'credit'; // cash | bank | credit
  bool _saving = false;

  final _items = <_ItemRowModel>[];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer: isDesktop
          ? null
          : const Drawer(
              child: YallaSidebar(currentRoute: '/purchases/raw/add'),
            ),
      body: isDesktop
          ? Row(
              children: [
...
class _ItemsHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: const [
        Expanded(flex: 4, child: Text('اسم المادة', style: style)),
        SizedBox(width: 8),
      ...
      children: [
        Expanded(
          ...
          child: TextField(
            ...
            decoration: const InputDecoration(
              hintText: 'مثال: صنفرة، تينر، معجون …',
              border: OutlineInputBorder(),
              isDense: true,
            ),
