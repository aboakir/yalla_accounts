// 📁 lib/features/cheques/screens/cheques_returned_screen.dart
//
// ChequesReturnedScreen — الشيكات الراجعة (RETURNED)
// ---------------------------------------------------
// - داتا حقيقية من جدول cheques.
// - فلترة status = 'RETURNED'.
// - بدون RTL (نستخدم TextAlign.right فقط).
// - Sidebar ثابت للديسكتوب، Drawer للموبايل.
// - تصميم متناسق مع Incoming / Outgoing / Postdated / Dashboard.
//
...
class ChequesReturnedScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      drawer: isDesktop
          ? null
          : const YallaSidebar(currentRoute: '/cheques/returned'),
      body: AdaptiveRow(
        children: [
          if (isDesktop) const YallaSidebar(currentRoute: '/cheques/returned'),
          const Expanded(child: _ReturnedBody()),
        ],
      ),
    );
  }
...
class _ReturnedBody extends StatelessWidget {
  const _ReturnedBody();
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snap) {
        ...
        // فلترة الشيكات الراجعة فقط (status = RETURNED / returned)
        List<Map<String, dynamic>> rows = (snap.data ?? [])
            .where((r) =>
                (r['status'] ?? '').toString().toUpperCase() == 'RETURNED')
            .toList();

        if (rows.isEmpty) {
          return const Center(
            child: Text(
              "لا توجد شيكات راجعة حالياً",
              textAlign: TextAlign.center,
